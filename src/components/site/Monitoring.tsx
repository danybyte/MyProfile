    ...
    <ul className="space-y-4">
      {sortedItems.map((item) => {
        ...
        const titleClass = item.unavailable
          ...
          : "line-clamp-2 font-display text-base leading-snug transition-colors group-hover:text-primary";
        const content = (
          <>
...
function Skeleton() {
  return (
    <ul className="space-y-4">
      {[0, 1, 2].map((i) => (
        <li key={i} className="border-b border-dashed border-border pb-4 last:border-b-0">
          <div className="h-4 w-3/4 animate-pulse bg-muted" />
          <div className="mt-3 h-2 w-1/3 animate-pulse bg-muted" />
        </li>
      ))}
    </ul>
  );
